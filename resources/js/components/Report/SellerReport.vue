			...
			<div class="panel-heading">
				<div class="pull-left col-sm-1">
					<div class="form-group">
						...
						<select @change="getSellerInfo()" class="form-control" style="position: absolute; top: 6px; left: 59px;" v-model="custom_row">
							<option :value="index" v-for="(row, index) in select_row" v-text="row"></option>
						</select>
					</div>
				</div>
				<div class="col-md-1"></div>
				<div class="col-md-2" v-show="showCustom">
					<input @change="getSellerInfo()" class="form-control search-box" id="from" type="date" v-model="from" />
				</div>
				<div class="col-md-2" v-show="showCustom">
					<input @change="getSellerInfo()" class="form-control search-box" id="to" type="date" v-model="to" />
				</div>
				<div class="col-md-2">
					<select @change="getSellerInfo()" class="form-control" v-model="seller_id">
						<option :value="value.value" v-for="(value, index) in sellers" v-text="value.text"></option>
					</select>
				</div>
				<div class="col-md-2">
					<select @change="getSellerInfo()" class="form-control" v-model="trx_type">
						<option value="1">PAYED</option>
						<option value="2">RECIEVED</option>
						<option value="3">PAYED & RECIEVED</option>
					</select>
				</div>
			</div>
			<div style="overflow-x: auto;background-color:#004a88">
				<table class="table table-bordered table-responsive">
					...
					<tbody>
						<tr :key="index" v-for="(data,index) in allData.dateWise">
							...
							<td class="text-center">{{ data.seller }}</td>
				...
				<div class="col-sm-2">
					<table class="table table-bordered">
						<tbody>
							<tr v-if="allData.expense">
								<td>Expense</td>
								<td>
									<b>{{allData.expense}}</b>
								</td>
							</tr>
							<tr v-if="allData.income">
								<td>Income</td>
								<td>
									<b>{{allData.income}}</b>
								</td>
